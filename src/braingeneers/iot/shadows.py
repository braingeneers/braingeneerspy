...
class DatabaseInteractor:
    ...
    @classmethod
    def _resolve_bool_setting(cls, explicit_value, generic_env_name, legacy_env_name=None, default_value=None):
        resolved = cls._parse_bool(explicit_value)
        if resolved is None:
            resolved = cls._parse_bool(os.getenv(generic_env_name))
        if resolved is None and legacy_env_name is not None:
            resolved = cls._parse_bool(os.getenv(legacy_env_name))
        if resolved is None:
    ...
    def __init__(
        self,
        credentials: Union[str, io.IOBase] = None,
        overwrite_endpoint=None,
        overwrite_api_key=None,
        jwt_service_token=None,
        session=None,
        use_auth=None,
        trust_env=None,
        timeout=30,
        ...
        self.use_auth = self._resolve_bool_setting(
            use_auth,
            "BRAINGENEERS_HTTP_USE_AUTH",
            default_value=True,
        )
        trust_env = self._resolve_bool_setting(
            trust_env,
            "BRAINGENEERS_HTTP_TRUST_ENV",
            legacy_env_name="BRAINGENEERS_SHADOWS_TRUST_ENV",
            default_value=True,
        )
    ...
    def _auth_headers(self):
        if not self.use_auth:
            return {}
        return {"Authorization": "Bearer " + self.jwt_service_token['access_token']}
    ...
    class __API_object:
        ...
        """
        def __init__(self, endpoint, api_token, api_object_id, jwt_service_token, request_fn, auth_headers_fn, timeout):
            ...
            self._auth_headers = auth_headers_fn
            self.timeout = timeout
        ...
        def spawn(self):
            ...
            if len(response.json()['data']) == 0:
                ...
                response = self._request("POST", api_url, json=data, headers=headers, timeout=self.timeout)
        ...
        def push(self):
            ...
            headers = self._auth_headers()
            data = {"data": self.attributes}
            response = self._request("PUT", url, headers=headers, json=data, timeout=self.timeout)
            self.parse_API_response(response.json()['data'])
    ...
    class __Thing(__API_object):
        def __init__(self, endpoint, api_token, jwt_service_token, request_fn, auth_headers_fn, timeout):
            super().__init__(endpoint, api_token, "interaction-things", jwt_service_token, request_fn, auth_headers_fn, timeout)
    ...
    class __Plate(__API_object):
        def __init__(self, endpoint, api_token, jwt_service_token, request_fn, auth_headers_fn, timeout):
            super().__init__(endpoint, api_token, "plates", jwt_service_token, request_fn, auth_headers_fn, timeout)
    ...
    class __Well(__API_object):
        def __init__(self, endpoint, api_token, jwt_service_token, request_fn, auth_headers_fn, timeout):
            super().__init__(endpoint, api_token, "wells", jwt_service_token, request_fn, auth_headers_fn, timeout)
        
    class __Sample(__API_object):
        def __init__(self, endpoint, api_token, jwt_service_token, request_fn, auth_headers_fn, timeout):
            super().__init__(endpoint, api_token, "samples", jwt_service_token, request_fn, auth_headers_fn, timeout)
    ...
    def create_interaction_thing(self, type, name):
        thing = self.__Thing(self.endpoint, self.token, self.jwt_service_token, self._request, self._auth_headers, self.timeout)
    ...
    def create_plate(self, name, rows, columns, image_params={}):
        plate = self.__Plate(self.endpoint, self.token, self.jwt_service_token, self._request, self._auth_headers, self.timeout)
        ...
        if len(plate.attributes["wells"]) == 0 or plate.attributes["wells"] is None:
            for i in range(1, rows+1):
                for j in range(1, columns+1):
                    well = self.__Well(self.endpoint, self.token, self.jwt_service_token, self._request, self._auth_headers, self.timeout)
    ...
    def create_experiment(self, name, description):
        experiment = self.__Experiment(self.endpoint, self.token, self.jwt_service_token, self._request, self._auth_headers, self.timeout)
    ...
    def get_sample(self, sample_id):
        sample = self.__Sample(self.endpoint, self.token, self.jwt_service_token, self._request, self._auth_headers, self.timeout)
        sample.id = sample_id
        sample.pull()
        return sample
